  ...
  if (item.key === "produtos") {
    return (
      <li className="relative group">
        ...
        {/* Mega panel */}
        <div className="hidden group-hover:block absolute left-0 top-full z-50 pt-0">
          <div className="min-w-[280px] rounded-b-md bg-white text-foreground shadow-lg border border-black/10 py-2">
            {cats.data?.length ? (
              cats.data.map((c) => (
                <Link
                  key={c.id}
              ...
              <div className="px-4 py-2 text-sm text-muted-foreground">Nenhuma categoria</div>
            )}
            <div className="border-t mt-1 pt-1">
              <Link to="/produtos" className="block px-4 py-2 text-sm font-medium text-primary hover:bg-primary/10">
